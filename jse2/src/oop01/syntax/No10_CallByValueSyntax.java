 ...
 * 문자열은 char*형 이므로 Call By Address 라고한다.
...
*/
/*String객체의 데이터 전달 유형
 * -String객체는 객체임으로 내부적으로 Call By Reference를 사용하나
 * -문자열 할당등의 조작시 무조건 Call By Value 기능으로 작동한다.
 * -문자열이 같으면 멤버변수나 지역변수는 같은 Hash Code값을 가지고있다
 * -그리고 둘의 구분은 영영 없어진다.
 * -그러나 새로운 문자열이 지역변수에 대입되면
 * -도 다른 HasgCode가 발생되면서 멤버면수와 구분되어 지게 된다.
 *  */
